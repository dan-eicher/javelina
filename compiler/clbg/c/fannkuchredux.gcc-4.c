 ...
 *
 * Unmodified fannkuchredux gcc-4, kept here as a MACHINE CALIBRATION point:
 * its published time on the game's i5-3330 divided by its time here is the
 * scaling constant for this box. Build it with the published command line,
 * which the program page gives as:
 *
 *   gcc -pipe -Wall -O3 -fomit-frame-pointer -march=ivybridge
 *
 * (stdlib.h is added: the published file calls exit/atoi without it, which
 * older gcc accepted implicitly and current gcc rejects. No code change.)
 */
