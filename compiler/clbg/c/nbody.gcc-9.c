 ...
 *
 * Unmodified nbody gcc-9, kept here as a MACHINE CALIBRATION point beside
 * fannkuchredux.gcc-4. Two of them rather than one because they stress
 * different hardware — this is AVX floating point, that one is SSE integer
 * shuffling — so the pair gives a RANGE for this box against the game's
 * i5-3330 rather than a single constant that hides which workload it measured.
 *
 * Published build command line, from the program page:
 *   gcc -pipe -Wall -O3 -fomit-frame-pointer -march=ivybridge
 */
