 ...
 *
 * burgc's generated matcher tiles the value sub-trees and walks the cg_jump
 * spine, emitting opcodes into the burg context's byte buffer, capped with the
 * structured body `end`. The caller owns the burg_ctx (and reads
 * ctx->emit.code for the bytes). */
